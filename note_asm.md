...
Préprossessing -> remplace les macro ou bout de code 
analyse syntasic du code soource$


Compilation -> traduction du code source en langage prossesseur (kernel windows/linux)

Assemblage -> traduction langage assembleur en code machine (OpCodes)

linhing : résolution des symboles et liens au librairies 
...
#### Compilation 
```bash
gcc -S -nasm=intel -o ASM_FILE.s input.c
```
```s
	.file	"code.c"
...
```bash
gcc -c -o assemblage.o compilation.s
```
```binary
Hello, world
...
Compile un code source vers un 

### fichier Object
### linker 
assemble les fichier object 

binaire statique possede toute les libraire
binaire dinamique a besoin des dll/dépendance
...
Def : petit emplacement mémoire CPU. mémoire les plus rappide des ordinateur. 

# node randome
 

appels sysèmes : le moyen le plus simple pour execcuter une tache en asb 
ils sont le(interface entre le mode dutilisateur et le mode noyau.
...
argc = nbr paramètre -> 
argv = tableau qui contien les adress en argument -> ** en c
